...
def get_best_model():
    """Fetch the best model from MLflow based on f1_score."""
    experiment_name = config.EXPERIMENT_NAME
    experiment = mlflow.get_experiment_by_name(experiment_name)
    experiment_id = experiment.experiment_id
    runs_df = mlflow.search_runs(experiment_ids=experiment_id, order_by=[
                                 'metrics.f1_score DESC'])
    best_run = runs_df.iloc[0]
    best_run_id = best_run['run_id']
    best_model = f'runs:/{best_run_id}{config.MODEL_NAME}'

    return mlflow.sklearn.load_model(best_model)


def generate_predictions(data_input):
    """Generate predictions for a single input."""
    loan_prediction_model = get_best_model()
...
if __name__ == '__main__':
    sample_input = []  # Sample input data for prediction
    generate_predictions(sample_input)
